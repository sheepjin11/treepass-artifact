 ...
 */
#  define PIVOT_META_SPECIAL   (0x80u)  // bit7
#  define PIVOT_META_CNT_MASK  (0x7Fu)  // bits[0..6]
 ...
 * Override at build time with -DPIVOT_CNT_OVERFLOW_THRESHOLD=N. */
#ifndef PIVOT_CNT_OVERFLOW_THRESHOLD
#   define PIVOT_CNT_OVERFLOW_THRESHOLD  PIVOT_META_CNT_MASK  // 127
#endif
...
static inline uint8_t
btree_pivot_meta_cnt7(const index_entry *entry)
{
    return (uint8_t)(btree_pivot_meta_load(entry) & PIVOT_META_CNT_MASK);
}

static inline void
btree_pivot_meta_mark_special(index_entry *entry)
{
    uint8_t *b0 = index_entry_child_addr_b0_ptr(entry);
    __atomic_fetch_or(b0, (uint8_t)PIVOT_META_SPECIAL, __ATOMIC_RELAXED);
}

static inline void
btree_pivot_meta_clear_special(index_entry *entry)
{
    uint8_t *b0 = index_entry_child_addr_b0_ptr(entry);
    __atomic_fetch_and(b0, (uint8_t)~PIVOT_META_SPECIAL, __ATOMIC_RELAXED);
}

/* Set CNT(7) to a specific value (clamped to 0x7F). Preserves SPECIAL bit. */
static inline void
btree_pivot_meta_set_count(index_entry *entry, uint8_t count)
{
    uint8_t *p   = index_entry_child_addr_b0_ptr(entry);
    uint8_t  old = __atomic_load_n(p, __ATOMIC_RELAXED);
    if (count > PIVOT_META_CNT_MASK) {
        count = PIVOT_META_CNT_MASK;
    }
    for (;;) {
        uint8_t newv = (uint8_t)((old & PIVOT_META_SPECIAL) | count);
 ...
 * Overflow is handled by the caller (btree_update_pivot_cnt_and_is_warm)
 * which resets all sibling counts and re-anchors entry_time. */
static inline uint8_t
btree_pivot_meta_cnt7_inc_atomic(index_entry *entry)
{
    uint8_t *p   = index_entry_child_addr_b0_ptr(entry);
    uint8_t  old = __atomic_load_n(p, __ATOMIC_RELAXED);

    for (;;) {
        if (old & PIVOT_META_SPECIAL) {
            return old; // sticky SPECIAL
        }
        uint8_t cnt = (uint8_t)(old & PIVOT_META_CNT_MASK);
        if (cnt >= PIVOT_META_CNT_MASK) {
            return old; // saturated, caller handles overflow reset
        }
        uint8_t newv = (uint8_t)((old & PIVOT_META_SPECIAL) | (uint8_t)(cnt + 1));
        if (__atomic_compare_exchange_n(p, &old, newv, /*weak=*/false,
                                        __ATOMIC_RELAXED, __ATOMIC_RELAXED)) {
            return newv;
        }
