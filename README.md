...
```
--threads        2         bench worker threads
--cache-mb       128       query-phase block cache size
--load-cache-mb  512       one-shot load-phase block cache size
--phase-ops      8M        timed ops per phase
--db-path        <repo>    directory that holds the SplinterDB data file
                           (defaults to the cloned repo root; pass an
                           NVMe mount like /mnt/nvme for paper-grade timing)
...
The default `--db-path` is the cloned repo root so the smoke runs
without extra setup; pass an explicit NVMe path for paper-grade
timing. Every knob above is a flag on `run_smoke.py` — see
`python3 run_smoke.py --help`.
