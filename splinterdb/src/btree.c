...
/* Pivot overflow reset counter */
_Atomic uint64_t pivot_overflow_reset_count = 0;

/*
 * Subtree access heatmap: tracks spatial distribution of lookups over time.
 * At btree root level, each lookup's child_idx is normalized to a spatial bin.
 * Time is divided into windows based on elapsed time since first access.
 * Enable with -DSUBTREE_ACCESS_HEATMAP at compile time.
 */

/*
 * Leaf access trace: records sampled leaf-level accesses to a file.
 * Each line: elapsed_ns,key_prefix_uint64
 * Sampling rate: 1 in LEAF_TRACE_SAMPLE_RATE accesses.
 * Enable with -DLEAF_ACCESS_TRACE at compile time.
 */
 ...
 *      ci_lt_ei flag to allocate with max clock when compaction is hot.
 *-----------------------------------------------------------------------------
 */
bool32
...
static inline bool
pivot_is_special(const index_entry *entry)
{
   return (btree_pivot_meta_load(entry) & PIVOT_META_SPECIAL) != 0;
}

// Read pivot hit-counter as u64
static inline uint64_t
pivot_hits_u64(index_entry *entry)
{
   return (uint64_t)btree_pivot_meta_cnt7(entry);
}

// Increment hit-counter unless SPECIAL is set (relaxed is fine here).
static inline void
pivot_cnt_inc_relaxed(index_entry *entry)
{
   (void)btree_pivot_meta_cnt7_inc_atomic(entry);
}

// Mark pivot SPECIAL
static inline void
pivot_mark_special(index_entry *entry)
{
   btree_pivot_meta_mark_special(entry);
}

// Clear pivot SPECIAL
static inline void
pivot_clear_special(index_entry *entry)
{
   btree_pivot_meta_clear_special(entry);
}

// Set pivot count directly
static inline void
pivot_set_count(index_entry *entry, uint8_t count)
{
   btree_pivot_meta_set_count(entry, count);
}
...
static inline bool32
btree_update_pivot_cnt_and_is_warm(cache *cc, const btree_config *cfg, btree_node *node, index_entry *entry)
{
   // 1) Check was_cached (special bit = "child was in cache before")
   //    If set, clear it and remember for compensation later.
   bool32 was_cached = pivot_is_special(entry);
   if (was_cached) {
      pivot_clear_special(entry);
   }

   // 2) Evict interval not ready => treat as warm (EI-gated retention)
   uint64 ei = cache_get_evict_interval_threshold(cc);
   if (ei == EI_NOT_READY) {
      pivot_mark_special(entry);
      return TRUE;
   }
   ...
   // 4) Compute elapsed since page enter, read current hits
   timestamp now     = platform_get_timestamp();
   uint64    elapsed = (uint64)(now - init);
   uint64    hits    = pivot_hits_u64(entry);
   double mult_d = cache_get_early_eviction_ei_multiplier(cc);

   // 4a) was_cached: compensate hits to max that keeps RI > EI (COLD)
   //     max_hits = floor(elapsed / (ei * mult)) - 1
   if (was_cached && ei > 0 && mult_d > 0) {
      uint64 ei_mult = (uint64)(ei * mult_d);
      if (ei_mult > 0) {
         uint64 threshold_hits = elapsed / ei_mult;  // floor
         uint64 max_hits = (threshold_hits > 0) ? (threshold_hits - 1) : 0;
         if (max_hits > hits) {
            pivot_set_count(entry, (uint8_t)(max_hits > 127 ? 127 : max_hits));
            hits = max_hits;
         }
      }
   }

   // 4b) hits == 0: always COLD
   if (hits == 0) {
      pivot_cnt_inc_relaxed(entry);
      return FALSE;
   }

   // 5) Threshold: elapsed <= EI * mult * hits
   if (ei == 0) {
      return TRUE;
   }

   long double lhs = (long double)elapsed;
   long double rhs = (long double)ei * (long double)hits * (long double)mult_d;

   if (lhs <= rhs) {
      pivot_mark_special(entry);
      return TRUE;
   } else {
      // COLD: normally increment, but if at overflow threshold,
      // reset all sibling pivots' counts and re-anchor entry_time instead.
      if (hits >= PIVOT_CNT_OVERFLOW_THRESHOLD) {
         uint64_t cnt = __atomic_add_fetch(&pivot_overflow_reset_count, 1, __ATOMIC_RELAXED);
         if ((cnt & (cnt - 1)) == 0) {  // power of 2: 1,2,4,8,...
            platform_error_log("[PT_OVERFLOW] reset #%lu: node_addr=%lu, "
                               "num_entries=%u, hits=%lu\n",
                               cnt, node->addr,
                               btree_num_entries(node->hdr), hits);
         }
                  ...
                  bool32             *out_is_warm)    // OUT (optional)
{
   ...
   for (h = btree_height(node.hdr); h > stop_at_height; h--) {
      ...
      if (type == PAGE_TYPE_BRANCH && TRUE) {
         child_in_cache = is_warm && cache_present_by_addr(cc, child_node.addr);
         if (!child_in_cache) {
            is_warm = is_warm && btree_update_pivot_cnt_and_is_warm(cc, cfg, &node, entry);
         }
      }
        ...
        state->h--)
   {
      ...
      if (state->type == PAGE_TYPE_BRANCH
         ...
         if (!state->child_in_cache) {
            state->is_warm =
               state->is_warm
               && btree_update_pivot_cnt_and_is_warm(
      ...
      if (!node_is_warm && state->type == PAGE_TYPE_BRANCH) {
         cache_unget_cold(state->cc, state->node.page);
...
btree_iterator_find_end(btree_iterator *itor)
{
   ...
   cache_unget_cold(itor->cc, end.page);
}
...
btree_iterator_next_leaf(btree_iterator *itor)
{
   ...
   cache_unget_cold(cc, itor->curr.page);
   itor->curr.page = NULL;
   itor->curr.hdr  = NULL;
          ...
          && itor->curr.hdr->generation != itor->end_generation)
   {
       ...
       */
      cache_unget_cold(itor->cc, itor->curr.page);
      itor->curr.page = NULL;
      itor->curr.hdr  = NULL;
...
btree_pack_setup_start(btree_pack_req *req)
{
   ...
   req->mini.ci_lt_ei = req->ci_lt_ei;
...
btree_pack_create_next_node(btree_pack_req *req, uint64 height, key pivot)
{
   ...
   // ci_lt_ei propagated to mini, so btree_alloc uses cache_alloc_compaction_hot.
   btree_alloc(req->cc,
               &req->mini,
               height,
               pivot,
   ...
   if (!req->ci_lt_ei
      ...
      uint32 alloc_clock = cache_get_page_clock(req->cc, new_node.addr, NULL);
      // Leaf (height==0): use avg of inputs directly -- can be 0 if inputs are cold.
      // Internal (height>0): max(inherit, alloc) with inherit min=1.
      uint32 final_clock;
      if (height == 0) {
         final_clock = inherit_clock;
      } else {
         if (inherit_clock < 1)
            inherit_clock = 1;
         final_clock = (inherit_clock > alloc_clock) ? inherit_clock : alloc_clock;
      }
                               ...
                               uint64             *cached_internal_pages)
{
   ...
   for (table_index k = 0; k < n; k++) {
      ...
      uint8_t cnt  = (uint8_t)(meta & PIVOT_META_CNT_MASK);
      (*total_pivots)++;
      *combined_sum    += cnt;
      *combined_sum_sq += (double)cnt * cnt;
      if (meta & PIVOT_META_SPECIAL) {
         (*special_count)++;
      } else {
         (*active_count)++;
         *active_sum    += cnt;
...
void btree_print_subtree_access_heatmap(platform_log_handle *log_handle) { (void)log_handle; }
void btree_reset_subtree_access_heatmap(void) {}

void btree_leaf_trace_open(const char *filepath) { (void)filepath; }
void btree_leaf_trace_close(void) {}
