...
struct trunk_pivot {
   ...
   bool32     ci_lt_ei;  // CI < EI: set at serialize time using per-node entry_time
   ondisk_key key;
};
...
typedef struct bundle_compaction {
   ...
   bool32                    ci_lt_ei;  // CI < EI at compaction enqueue time
} bundle_compaction;
 ...
 */
#define TRUNK_PIVOT_COMP_CNT_MASK       ((uint64)0xFFu)
#define TRUNK_PIVOT_ADDR_MASK           (~TRUNK_PIVOT_COMP_CNT_MASK)
#define TRUNK_PIVOT_COMP_CNT_OVERFLOW   (0xFFu)  // reset threshold
...
static inline uint8
trunk_pivot_comp_cnt(const trunk_pivot *pivot)
{
   return (uint8)(pivot->child_addr & TRUNK_PIVOT_COMP_CNT_MASK);
}

static inline uint8
trunk_ondisk_pivot_comp_cnt(const trunk_ondisk_pivot *pivot)
{
   return (uint8)(pivot->child_addr & TRUNK_PIVOT_COMP_CNT_MASK);
}

static inline void
trunk_pivot_set_comp_cnt(trunk_pivot *pivot, uint8 cnt)
{
                   ...
                   trunk_pivot_stats stats)
{
   ...
   result->child_addr            = child_addr;  // includes comp_cnt in low byte
                ...
                trunk_ondisk_pivot *dest)
{
   ...
   dest->child_addr = pvt->child_addr;  // includes comp_cnt in low byte
...
trunk_node_serialize(trunk_context *context, trunk_node *node, uint64 old_addr)
{
   ...
   // Done here while we hold header_page, avoiding a separate addr-based API.
   {
      ...
      for (uint64 p = 0; p < np; p++) {
         if (trunk_pivot_comp_cnt(trunk_node_pivot(node, p))
             >= TRUNK_PIVOT_COMP_CNT_OVERFLOW) {
            overflow = TRUE;
            break;
         }
      }
      if (overflow) {
         for (uint64 p = 0; p < np; p++) {
            trunk_pivot_set_comp_cnt(trunk_node_pivot(node, p), 0);
         }
                ...
                uint64                        split_source_addr)
{
   ...
   for (uint64 i = 0; i < num_nodes; i++) {
      ...
      // address is known.
      {
         ...
         uint64    ei  = cache_get_evict_interval_threshold(context->cc);
         if (TRUE
             && node_entry_time != 0 && ei != 0 && now > node_entry_time) {
            ...
            for (uint64 p = 0; p < num_children; p++) {
               ...
               uint8        comp_cnt = trunk_pivot_comp_cnt(pvt);
               if (comp_cnt == 0) {
                  continue;
               }
               timestamp ci  = elapsed / comp_cnt;
               pvt->ci_lt_ei = (ci <= ei);
                                      ...
                                      void          *arg)
{
   ...
   for (uint64 i = 0; i < trunk_node_num_children(target); i++) {
      ...
      if (pivot_matches_compaction(context, target, i, args)) {
         ...
         // Increment compaction counter (saturates at TRUNK_PIVOT_COMP_CNT_OVERFLOW;
         // overflow reset happens in serialize_nodes when the node is written)
         uint8 cnt = trunk_pivot_comp_cnt(pvt);
         if (cnt < TRUNK_PIVOT_COMP_CNT_OVERFLOW) {
            trunk_pivot_set_comp_cnt(pvt, cnt + 1);
         }
...
maplet_compaction_task(void *arg, void *scratch)
{
   ...
   while (bc != NULL && bc->state == BUNDLE_COMPACTION_SUCCEEDED) {
      if (!branch_is_null(bc->output_branch)) {
         ...
         bool32 maplet_ci_lt_ei = bc->ci_lt_ei;
                   ...
                   incorporation_tasks          *itasks)
{
   ...
   uint8 parent_comp_cnt = trunk_pivot_comp_cnt(pvt);
   ...
   uint8 child_comp_cnt = (num_children > 0) ? (parent_comp_cnt / num_children) : 0;
   ...
   for (uint64 j = 0; j < num_children; j++) {
      ...
      trunk_pivot_set_comp_cnt(new_pivot, child_comp_cnt);
   }
                            ...
                            const trunk_context *context)
{
   ...
   // Pivot tracking overflow reset count
   extern _Atomic uint64_t pivot_overflow_reset_count;
   uint64_t overflow_resets = __atomic_load_n(&pivot_overflow_reset_count, __ATOMIC_RELAXED);
   platform_log(log_handle, "\nPivot Tracking Overflow Resets: %lu\n", overflow_resets);
}
