...
typedef uint64 (*get_ei_thre_fn)(const cache *cc);          // Get EI threshold
typedef bool32 (*get_enable_ei_fn)(const cache *cc);        // EI-based decision enabled?
typedef double (*get_early_eviction_ei_mult_fn)(const cache *cc);
...
typedef struct cache_ops {
   ...
   page_generic_fn            page_unget_cold;
   ...
   get_ei_thre_fn       get_ei_thre;
   get_enable_ei_fn     get_enable_ei;
   get_early_eviction_ei_mult_fn   get_early_eviction_ei_mult;
 ...
 * cache_unget_cold
 *
 * Drop a reference and attempt early eviction of a COLD page.
 * Called by lookup path when the page is determined to be cold.
 *----------------------------------------------------------------------
 */
static inline void
cache_unget_cold(cache *cc, page_handle *page)
{
   return cc->ops->page_unget_cold(cc, page);
}
 ...
 * cache_mark_page_hot
 *
 * Mark a page as hot (upgrade to NODE_CLASS_HOT).
 * Used when a child node transitions from COLD to WARM to reduce
 * metadata overhead by marking the parent node as hot.
 *----------------------------------------------------------------------
 */
static inline void
...
/*
 *-----------------------------------------------------------------------------
 * cache_get_evict_interval_threshold
 *
 * Returns the evict interval threshold for the cache.
 *-----------------------------------------------------------------------------
 */
static inline uint64
cache_get_evict_interval_threshold(const cache *cc)
{
   return cc->ops->get_ei_thre(cc);
}
...
cache_get_early_eviction_ei_multiplier(const cache *cc) {
  return cc->ops->get_early_eviction_ei_mult ? cc->ops->get_early_eviction_ei_mult(cc) : 1.0;
}
