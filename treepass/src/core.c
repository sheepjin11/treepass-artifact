...
core_memtable_compact(core_handle *spl, uint64 generation, const threadid tid)
{
   ...
   uint64 memtable_ei = cache_get_evict_interval_threshold(spl->cc);
   bool32 memtable_ci_lt_ei =
      (TRUE
       && memtable_ci_ns > 0 && memtable_ei != 0
       && memtable_ci_ns <= memtable_ei);

   if (memtable_ci_lt_ei) {
   ...
   req.ci_lt_ei = memtable_ci_lt_ei;
   if (!req.ci_lt_ei) {
...
core_lookup_async(core_lookup_async_state *state)
{
   ...
   // makes all pages look COLD to btree_update_pivot_cnt_and_is_warm.
