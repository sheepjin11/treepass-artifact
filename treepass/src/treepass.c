...
/*
 * Helper: derive node_class from page_type and entry_time for cache tracing.
 */
static inline uint8
treepass_trace_node_class(page_type type, timestamp entry_time)
{
   if (type == PAGE_TYPE_BRANCH) {
      return (entry_time != 0) ? NODE_CLASS_WARM : NODE_CLASS_COLD;
   }
   if (type == PAGE_TYPE_FILTER) {
      return NODE_CLASS_META;
   }
   return NODE_CLASS_HOT;
}
...
// Forward declaration for EI threshold function
static uint64 treepass_get_evict_interval_threshold(const treepass *hc);
   ...
   for (entry_no = start_entry_no; entry_no < end_entry_no; entry_no++) {
          ...
          && treepass_try_set_writeback(cc, entry_no, num_passes))
      {
         ...
         for (i = 0; i < req_count; i++) {
            ...
            CACHE_TRACE_RECORD(cc->trace,
                               is_flush ? CACHE_TRACE_SHUTDOWN_WB
                                        : CACHE_TRACE_WRITEBACK,
                               addr,
                               next_entry->type,
                               0,
                               treepass_trace_node_class(next_entry->type,
                                                        next_entry->page.entry_time));
                  ...
                  bool32 ee_pass)
{
   ...
   CACHE_TRACE_RECORD(cc->trace,
                      ...
                      addr,
                      entry->type,
                      CACHE_TRACE_FLAG_EVICT,
                      treepass_trace_node_class(entry->type, entry->page.entry_time));
...
treepass_evict_batch(treepass *cc, uint32 batch, bool32 first_pass)
{
   ...
   uint64 ei = __atomic_load_n(&cc->evict_interval_thre, __ATOMIC_RELAXED);
   ...
   __atomic_store_n(&cc->evict_interval_thre, new_ei, __ATOMIC_RELEASE);
...
treepass_move_hand(treepass *cc, int64 num_passes)
{
   ...
   /* EE pass first: try to evict cold (entry_time==0) pages without touching
    * clock bits. If anything was evicted, the caller will find FREE slots
    * in this batch — skip the normal clock-decrementing pass. Otherwise,
    * fall through to standard batch eviction. */
   if (TRUE
       && treepass_evict_batch_ee(cc, batch_idx) > 0) {
...
treepass_alloc(treepass *cc, uint64 addr, page_type type)
{
   ...
   CACHE_TRACE_RECORD(
      cc->trace, CACHE_TRACE_ALLOC, entry->page.disk_addr, type, 0,
      treepass_trace_node_class(type, entry->page.entry_time));
...
treepass_try_page_discard(treepass *cc, uint64 addr)
{
   ...
   while (TRUE) {
      ...
      CACHE_TRACE_RECORD(
         ...
         addr,
         entry->type,
         (!(entry->status & TP_CLEAN)) ? CACHE_TRACE_FLAG_DIRTY : 0,
         treepass_trace_node_class(entry->type, entry->page.entry_time));
      uint64 lookup_no      = treepass_divide_by_page_size(cc, addr);
      cc->lookup[lookup_no] = TP_UNMAPPED_ENTRY;
                         ...
                         page_handle **page) // OUT
{
   ...
   uint64 miss_start, evict_elapsed, io_elapsed, finish_elapsed;
   if (cc->cfg->use_stats) {
   ...
   CACHE_TRACE_RECORD(cc->trace, CACHE_TRACE_LOAD, addr, type, 0,
      treepass_trace_node_class(type, entry->page.entry_time));
...
treepass_get_in_cache_async(treepass_get_async_state *state, uint64 depth)
{
   ...
   CACHE_TRACE_RECORD(
      ...
      state->addr,
      state->type,
      (!(state->entry->status & TP_CLEAN)) ? CACHE_TRACE_FLAG_DIRTY : 0,
      treepass_trace_node_class(state->type, state->entry->page.entry_time));
...
treepass_get_from_disk_async(treepass_get_async_state *state, uint64 depth)
{
   ...
   CACHE_TRACE_RECORD(
      state->cc->trace, CACHE_TRACE_LOAD, state->addr, state->type, 0,
      treepass_trace_node_class(state->type, state->entry->page.entry_time));
...
/*
 * treepass_unget_cold -- unget + early eviction for COLD pages.
 * Called by lookup path when page is determined to be cold (is_warm=FALSE).
 * Clears clock bits and attempts immediate eviction.
 */
void
treepass_unget_cold(treepass *cc, page_handle *page)
{
   uint32         entry_number = treepass_page_to_entry_number(cc, page);
   const threadid tid          = platform_get_tid();
   ...
   treepass_log(page->disk_addr,
                  entry_number,
                  "unget_cold: entry %u addr %lu rc %u\n",
                  entry_number,
                  page->disk_addr,
                  treepass_get_ref(cc, entry_number, tid) - 1);
   treepass_dec_ref(cc, entry_number, tid);

   /* Batch-defer EE: don't try_evict here. Cold pages (entry_time==0) get
    * preferentially evicted in get_free_page's EE pass on the batch. */
}
   ...
   for (uint64 page_off = 0; page_off < pages_per_extent; page_off++) {
      ...
      switch (get_read_rc) {
         ...
         case GET_RC_EVICTED:
         {
            ...
            entry->page.disk_addr   = addr;
            entry->type             = type;
            // entry_time intentionally not set for prefetched pages:
            // prefetch is used for compaction input reads (cold path),
            // so pages should remain COLD (entry_time=0) and be eligible for EE.
            uint64 lookup_no        = treepass_divide_by_page_size(cc, addr);
            if (__sync_bool_compare_and_swap(
                   &cc->lookup[lookup_no], TP_UNMAPPED_ENTRY, free_entry_no))
            {
               ...
               CACHE_TRACE_RECORD(
                  cc->trace, CACHE_TRACE_LOAD, addr, type, 0,
                  treepass_trace_node_class(type, entry->page.entry_time));
               treepass_log(addr,
                              entry_no,
...
void
treepass_unget_cold_virtual(cache *c, page_handle *page)
{
   treepass *cc = (treepass *)c;
   treepass_unget_cold(cc, page);
}
...
treepass_alloc_compaction_hot(treepass *hc, uint64 addr, page_type type)
{
   ...
   CACHE_TRACE_RECORD(
      hc->trace, CACHE_TRACE_ALLOC, entry->page.disk_addr, type, 0,
      treepass_trace_node_class(type, entry->page.entry_time));
...
static uint64
treepass_get_evict_interval_threshold(const treepass *hc)
{
   return (__atomic_load_n(&hc->evict_interval_thre, __ATOMIC_RELAXED));
}
...
uint64
treepass_get_evict_interval_threshold_virtual(const cache *c)
{
   const treepass *hc = (const treepass *)c;
   return treepass_get_evict_interval_threshold(hc);
}

double
treepass_get_early_eviction_ei_multiplier_virtual(const cache *c)
{
   const treepass *hc = (const treepass *)c;
   uint64 mult_x100 = __atomic_load_n(&hc->early_eviction_ei_mult_x100, __ATOMIC_RELAXED);
   return (double)mult_x100 / 100.0;
}
...
static cache_ops treepass_ops = {
   ...
   .page_unget_cold      = treepass_unget_cold_virtual,
   ...
   .get_ei_thre       = treepass_get_evict_interval_threshold_virtual,
   .get_early_eviction_ei_mult   = treepass_get_early_eviction_ei_multiplier_virtual,
                       ...
                       bool32             use_cache_stats)
{
   ...
   // TreePass extensions
   cache_cfg->early_eviction_ei_mult = 1.0;
                ...
                platform_module_id mid)  // IN
{
   ...
   cc->evict_interval_thre = EI_NOT_READY;
   cc->early_eviction_ei_mult_x100 = (uint64)(cc->cfg->early_eviction_ei_mult * 100);
   ...
   cc->trace = NULL;
#ifdef CACHE_TRACE
   cc->trace = TYPED_MALLOC(cc->heap_id, cc->trace);
   if (cc->trace) {
      cache_trace_init(cc->trace, NULL);
   }
#endif
